<script lang="ts">
  ...
  import "./index.scss";
  import ColorList from "../color-list/index.svelte";
  ...
  function onNumInput(v: TInputEvent, key: keyof IConfig["options"], min: number, max: number) {
...
<div class="app-action-wrap">
  <div class="app-action-left-wrap">
    ...
    <ActionItem {labelWidth} title="图片质量">
      <svelte:fragment slot="popup">JPEG图片质量</svelte:fragment>
      <input
        class="bg-rate-input"
        style="width: 40px;"
        type="text"
        value={$config.options.quality}
        on:input={(v) => onNumInput(v, "quality", 0, 100)}
        on:change={(v) => onNumInputChange(v, "quality")}
      />
    </ActionItem>
    ...
    {#if $config.options.solid_bg}
      <ActionItem {labelWidth} title="背景颜色">
        <!-- <svelte:fragment slot="popup">使用纯色背景，默认使用图片模糊做背景</svelte:fragment> -->
  ...
  <div class="app-action-right-wrap">
    ...
    <ActionItem {labelWidth} title="圆角大小">
      ...
      <input class="bg-rate-input" type="text" value={$config.options.radius} on:input={(v) => onNumInput(v, "radius", 0, 30)} on:change={(v) => onNumInputChange(v, "radius")} />
    </ActionItem>
    ...
    <ActionItem {labelWidth} title="阴影大小">
      ...
      <input class="bg-rate-input" type="text" value={$config.options.shadow} on:input={(v) => onNumInput(v, "shadow", 0, 50)} on:change={(v) => onNumInputChange(v, "shadow")} />
    </ActionItem>
  </div>
</div>
